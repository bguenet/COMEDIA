...
#Do you want to use the full model with plankton module?
full=FALSE
...
#        if (t<time_exp_begin) {C13_CO2ATM=C13_CO2ATM} else {C13_CO2ATM=A13_co2atm*CO2ATM}
        ...
        #DOC inputs
#        if (t>time_exp_begin) {DOC[t]<-DOC[t]+DOC[time_exp_begin]*0.02}
#        if (t>time_exp_begin) {C13_DOC[t]<-C13_DOC[t]+C13_DOC[time_exp_begin]*0.02}       
       ...
       K2=10^-pK2
        ...
        #Henry's constant calculation
        H = (3.4*10^(-2))*exp(2400*((1/Temp)-(1/298.15)))  #H = 0.8317 #fixed for 25°C for the moment but must be temperature dependent
...
#mineralization_doc =0
        ...
        photosynthesis = (( rtheta_I  * fc[t])*(1-(PHYTO[t])/K))*(PHYTO[t])
#photosynthesis = 0
        #When the population is growing exudation is just a fraction of photosynthesis but once the maximum
        #carrying capacity is reached all the C fixed is exudated to take into account the fact that algae can't 
        #regulate the photosynthesis activity 
        ...
        aut_resp = CUEphyto* photosynthesis	
        phyto_death=Deathphyto * (PHYTO[t])
        #decomp_death = Deathdecomp*DECOMP[t]
        het_resp = (mineralization_poc+ mineralization_doc)*CUEdecomp
        ...
        #update of the pool
        DIC[t+1] = DIC[t] + aut_resp + het_resp - photosynthesis 
        PHYTO[t+1] = PHYTO[t] + photosynthesis - phyto_death -exudation - aut_resp
        #DECOMP[t+1] = DECOMP[t]+ mineralization_doc + mineralization_poc - decomp_death - het_resp
        DOC[t+1] = DOC[t]+ exudation - mineralization_doc
        POC[t+1] = POC[t]+phyto_death + (1-CUEdecomp)*mineralization_doc - mineralization_poc
        CO2ATM[t+1] = DIC[t+1]/(12.01 * H*10^(-6))
        diff_CO2[t+1]=CO2ATM[t+1]-CO2ATM[t]
        CO2ATM[t+1] = CO2atm
        #Whereas all is gC l-1 every *CO2* variables are in ppm
        Input_CO2[t+1]= CO2atm - abs(diff_CO2[t+1])
...
#C13_mineralization_doc =0
        ...
        C13_photosynthesis = photosynthesis*(C13_PHYTO[t]/PHYTO[t])
#C13_photosynthesis = 0
        #When the population is growing exudation is just a fraction of photosynthesis but once the maximum
        #carrying capacity is reached all the C fixed is exudated to take into account the fact that algae can't 
        #regulate the photosynthesis activity 
        if (PHYTO[t] < K-K/100) {
        ...
        C13_exudation = exudation*(C13_PHYTO[t]/PHYTO[t]) 
        }
        C13_aut_resp = CUEphyto* C13_photosynthesis	
        C13_phyto_death=Deathphyto * (C13_PHYTO[t])
        #C13_decomp_death = Deathdecomp*C13_DECOMP[t]
        C13_het_resp = (C13_mineralization_poc+ C13_mineralization_doc)*CUEdecomp
        ...
        #update of the 13C pool
        C13_DIC[t] = C13_CO2ATM[t] * 10^(-6) * H * 12.01 + (discrimination_CO2g_DIC*CO2ATM[t] * 10^(-6) * H * 12.01)
        C13_DIC[t+1] = C13_DIC[t] + C13_aut_resp + discrimination_aut_resp*PHYTO[t] + 
                                                       #C13_het_resp + discrimination_het_resp*DECOMP[t] - 
                                                       C13_het_resp + (discrimination_het_resp + discrimination_DOC_mine)*DOC[t] - 
                                                       C13_photosynthesis - discrimination_photosynthesis*DIC[t] 
                                                       
        C13_PHYTO[t+1] = C13_PHYTO[t] +  C13_photosynthesis + discrimination_photosynthesis*DIC[t] - 
                                                                    C13_phyto_death - discrimination_phyto_death*PHYTO[t] - 
                                                                    C13_exudation  - discrimination_exudation*PHYTO[t] -
                                                                    C13_aut_resp -  discrimination_aut_resp*PHYTO[t] 
        ...
        C13_POC[t+1] = C13_POC[t]+ C13_phyto_death + discrimination_phyto_death*PHYTO[t] + 
                                                       C13_decomp_death + (discrimination_decomp_death+ discrimination_DOC_mine)*DOC[t] - 
                                                       C13_mineralization_poc - discrimination_POC_mine*POC[t]
                                                       
        C13_CO2ATM[t+1] = (C13_DIC[t+1]/(12.01 * H*10^(-6))) - discrimination_CO2g_DIC*(DIC[t+1]/(12.01 * H*10^(-6)))
        
        C13_diff_CO2[t+1]=C13_CO2ATM[t+1] - C13_CO2ATM[t]

#The following line assumes that the flux of 13C_CO2 coming from the water is negligible
        #C13_CO2ATM[t+1] = C13_CO2ATM[t]  + A13_CO2atm *abs(diff_CO2[t])
        C13_CO2ATM[t+1] = A13_CO2atm*CO2ATM[t]
              
	                                                 }

        # conversion from ppm to gC l-1
        diff_CO2 = (12.01*101.325*(diff_CO2*10^-6 )*(10^-3))/(8.31441*Temp)
                           }	
...
#        R = ((delta/1000) +1)*0.0112372
#        A = R/(1+R)
#        delta = (((A/(1-A))/0.0112372)-1)*1000
#        delta_CO2aq[t]= delta_CO2g[t] + discrimination_CO2g_CO2aq
        
  #ATTENTION BCP DE FLUX SONT DEFINI PAR C_TOTAL IL NE FAUT LES CORRIGER COMME J'AI FAIT POUR LE DECOMP DEATH ET PHYTO DEATH      
...
#The following line assumes that the flux of 13C_CO2 coming from the water is negligible
        ...
        #IL FAUT VERIFIER LA DISCRIMINATION PENDANT LA PHASE CO2G A CO2AQ (pour ce dernier ça marche si on fixe le 13C co2 atm) ET AUSSI POUR LES CHANGEMENTS DE FORMES DE DIC. Peut être fixé le 13Catm dans un premier temps pour pouvoir mettre en place toute la structure et ensuite le faire changer.
	                                                 }
